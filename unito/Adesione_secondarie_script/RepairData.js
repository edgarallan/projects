...
var RepairDataService = (function() {
  ...
  function repairAssignmentsFromSheet() {
    ...
    const response = ui.alert('RIPARAZIONE DATI', 
      'Questo script leggerà TUTTE le righe del foglio SECONDARIE. ' +
    ...
    try {
      const ssSec = Utilities_.findSpreadsheetByName(CONFIG.REPOPULATE_SOURCE_FILE_SECONDARY);
      const sheet = ssSec.getSheetByName(CONFIG.REPOPULATE_SOURCE_SHEET_NAME);
      ...
      Logger.log('Caricamento record esistenti da Firebase per evitare duplicati...');
      ...
      let existingIdsRecovered = 0;
      const localTrackingMap = new Set();
      ...
      data.forEach((row, index) => {
        ...
        // Verifica duplicati nella stessa sessione di riparazione
        if (!fid && localTrackingMap.has(key)) {
          Logger.log(`⚠️ Riga ${index + 2}: Salto riga duplicata per ${email} al ${tsSheet}`);
          return;
        }

        // Se manca il firebase_id nel foglio, cerchiamo di recuperarlo o ne generiamo uno
        if (!fid) {
          ...
          cell.setDataValidation(null); // Rimuove la validazione che blocca la scrittura
          cell.setValue(fid);
          localTrackingMap.add(key);
        } else {
          localTrackingMap.add(key);
        }
      });

      // Eseguiamo il sync standard dopo aver assicurato che tutti abbiano un ID
      const stats = SheetSyncService.syncProposalsFromFile(ssSec, CONFIG.ASSEGNAZIONI_SECONDARIE_NODE);

      ui.alert(`Riparazione completata!\n\nNuovi ID generati: ${newIdsGenerated}\nID recuperati: ${existingIdsRecovered}\nAssegnazioni syncate: ${stats.successCount}`);
